...
?>
	...
	<section id="post-<?php the_ID(); ?>" <?php post_class('card'); ?>>
		<header class="entry-header">
			<?php
				if ( has_post_thumbnail() ) {
					the_post_thumbnail();
				} elseif ( isset( $team_member_email ) && ( '' != $team_member_email ) ) {
					echo '<figure class="jklmjklms" itemprop="image">' .  get_avatar( $team_member_email, 250 ) . '</figure>';
				}
			?>
			<h1 class="entry-title ggzg">
				<?php if ( is_single() ) : ?>
					<?php
